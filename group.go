...
type Group struct {
	...
	hide      func() bool
	active    bool
	hasDarkBg bool // TODO is this used?
}
...
func NewGroup(fields ...Field) *Group {
	...
	// TODO what is fullHeight vs rawHeight
	group.width = 80
	height := group.rawHeight()
	v := viewport.New(viewport.WithWidth(group.width), viewport.WithHeight(height)) //nolint:mnd
	group.viewport = v
	group.height = height
...
func (g *Group) getTheme() Theme {
	...
	return ThemeFunc(ThemeCharm)
}

func (g *Group) styles() GroupStyles {
	styles := g.getTheme().Theme(g.hasDarkBg)
	return styles.Group
}
...
func (g *Group) getContent() (int, string) {
	...
	gap := g.getTheme().Theme(g.hasDarkBg).FieldSeparator.Render()
	...
	if g.showErrors {
		for _, err := range errors {
			...
			parts = append(parts, wrap(
				g.getTheme().Theme(g.hasDarkBg).Focused.ErrorMessage.Render(err.Error()),
				g.width,
			))
		}
	}
